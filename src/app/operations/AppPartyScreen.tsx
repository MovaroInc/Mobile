...
export default function AddPartyScreen() {
  ...
  const mode: Mode = (route?.params?.mode ?? 'customer') as Mode;
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (mode === 'customer') {
        const { success, data, error, message } = await createCustomer(payload);
        if (!data?.id) throw new Error(message || 'Failed to create customer');
        Alert.alert('Saved', 'Customer created');
        nav.goBack();
      } else {
        const { success, data, error, message } = await createVendor(payload);
        if (!data?.id) throw new Error(message || 'Failed to create vendor');
